...
class progtrabController extends Controller
{
    ...
    public function actionBuscarProgtrab(Request $request)
    {
        ...
        if(session()->get('rango') !== '0'){    
            $regosc     = regOscModel::select('OSC_ID', 'OSC_DESC','OSC_STATUS')
                          ->get();                                 
            $regprogtrab= regProgtrabModel::join('ONGS','ONGS.OSC_ID','=','ONG_PROGRAMA_ETRABAJO.OSC_ID')
                          ->select( 'ONGS.OSC_DESC','ONG_PROGRAMA_ETRABAJO.*')
                          ->orderBy('ONG_PROGRAMA_ETRABAJO.PERIODO_ID','ASC')
                          ->orderBy('ONG_PROGRAMA_ETRABAJO.OSC_ID'    ,'ASC')
    ...
    public function actionVerProgtrab(){
        ...
        $regprogdtrab = regProgdtrabModel::select('FOLIO','PARTIDA','PERIODO_ID','OSC_ID','DFECHA_ELAB',
                        'DFECHA_ELAB2','DFECHA_ELAB3','PROGRAMA_ID','PROGRAMA_DESC','ACTIVIDAD_ID',
        ...
        if(session()->get('rango') !== '0'){  
            ...
            $totactivs  =regprogtrabModel::join('ONG_PROGRAMA_DTRABAJO','ONG_PROGRAMA_DTRABAJO.FOLIO', '=', 
                         ...
                         ->groupBy(  'ONG_PROGRAMA_ETRABAJO.PERIODO_ID','ONG_PROGRAMA_ETRABAJO.FOLIO')
                         ->get();                   
            $regprogtrab=regProgtrabModel::select('FOLIO','PERIODO_ID','OSC_ID','FECHA_ELAB','FECHA_ELAB2','FECHA_ELAB3',
    ...
    public function actionActualizarProgtrab(progtrabRequest $request, $id){
        ...
        if($regprogtrab->count() <= 0)
            ...
            $regprogtrab = regProgtrabModel::where('FOLIO',$id)        
                ...
                'FECHA_ELAB3'   => date('Y/m/d', strtotime(trim($dia1[0]->dia_desc.'/'.$mes1[0]->mes_mes.'/'.$request->periodo_id1) )),                
    ...
    public function IapxRubro(){
        ...
        $regtotxrubro=regOscModel::join('ONG_CAT_RUBROS','ONG_CAT_RUBROS.UMEDIDA_ID','=','ONGS.UMEDIDA_ID')
                      ->selectRaw('COUNT(*) AS TOTALXRUBRO')
                            ->get();
        $regosc=regOscModel::join('ONG_CAT_RUBROS','ONG_CAT_RUBROS.UMEDIDA_ID','=','ONGS.UMEDIDA_ID')
                      ->selectRaw('ONGS.UMEDIDA_ID,  ONG_CAT_RUBROS.RUBRO_DESC AS RUBRO, COUNT(*) AS TOTAL')
                        ->groupBy('ONGS.UMEDIDA_ID','ONG_CAT_RUBROS.RUBRO_DESC')
                        ->orderBy('ONGS.UMEDIDA_ID','asc')
                        ->get();
        //$procesos = procesosModel::join('SCI_TIPO_PROCESO','SCI_PROCESOS.CVE_TIPO_PROC','=','SCI_TIPO_PROCESO.CVE_TIPO_PROC')
        //    ->selectRaw('SCI_TIPO_PROCESO.DESC_TIPO_PROC AS TIPO, COUNT(SCI_PROCESOS.CVE_TIPO_PROC) AS TOTAL')
        //    ->groupBy('SCI_TIPO_PROCESO.DESC_TIPO_PROC')
    ...
    public function actionAltaNuevoProgdtrab(Request $request){
        ...
        //else{  
            ...
            $nuevoprogdtrab->DFECHA_ELAB3  = date('Y/m/d', strtotime(trim($dia1[0]->dia_desc.'/'.$mes1[0]->mes_mes.'/'.$request->periodo_id1) ));            
    ...
    public function actionActualizarProgdtrab(progdtrabRequest $request, $id, $id2){
        ...
        if($regprogdtrab->count() <= 0)
            ...
            $regprogdtrab = regProgdtrabModel::where(['FOLIO' => $id, 'PARTIDA' => $id2])        
                ...
                'DFECHA_ELAB3'   => date('Y/m/d', strtotime(trim($dia1[0]->dia_desc.'/'.$mes1[0]->mes_mes.'/'.$request->periodo_id1) )),                
