    ...
    public function actionBuscarJur(Request $request){
        ...
        if(session()->get('rango') !== '0'){
            $regosc       = regOscModel::select('OSC_ID', 'OSC_DESC','OSC_STATUS')
                            ->get();            
            $regjuridico  = regReqJuridicoModel::join('PE_OSC','PE_OSC.OSC_ID','=','ONG_REQ_JURIDICOS.OSC_ID')
                            ->select( 'PE_OSC.OSC_DESC','ONG_REQ_JURIDICOS.*')
                            ->orderBy('ONG_REQ_JURIDICOS.OSC_ID','ASC')
                            ->orderBy('ONG_REQ_JURIDICOS.PERIODO_ID','ASC')
                            ->orderBy('ONG_REQ_JURIDICOS.OSC_FOLIO' ,'ASC')
            ...
            $regjuridico  = regReqJuridicoModel::join('PE_OSC','PE_OSC.OSC_ID','=','ONG_REQ_JURIDICOS.OSC_ID')
                            ->select( 'PE_OSC.OSC_DESC','ONG_REQ_JURIDICOS.*')
                            ->where(  'ONG_REQ_JURIDICOS.OSC_ID',$arbol_id)
                            ->orderBy('ONG_REQ_JURIDICOS.OSC_ID'    ,'ASC')
                            ->orderBy('ONG_REQ_JURIDICOS.PERIODO_ID','ASC')
                            ->orderBy('ONG_REQ_JURIDICOS.OSC_FOLIO' ,'ASC') 
                            ->fper($fper)           //Metodos personalizados es equvalente a ->where('OSC_DESC', 'LIKE', "%$name%");
